...
async function getFinancialAdvice(payload){
  ...
  // ADVISOR_MODEL is a reasoning model — its hidden reasoning tokens count against max_tokens,
  // so a low limit can truncate the JSON answer mid-string. Give it headroom and retry once on a parse failure.
  const opts = {max_tokens: 1600, temperature: 0.3, response_format: {type:'json_object'}};
  try{
    return JSON.parse(await callOpenRouterAdvisor(messages, opts));
  }catch(err){
    return JSON.parse(await callOpenRouterAdvisor(messages, opts));
  }
}
