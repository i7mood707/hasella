...
async function classifyAutomationIntent(question, history){
  ...
  try{
    const raw = await callOpenAI(messages, {max_tokens: 120, temperature: 0, response_format: {type:'json_object'}});
...
async function classifyDomesticWorkerIntent(question, history){
  ...
  try{
    const raw = await callOpenAI(messages, {max_tokens: 40, temperature: 0, response_format: {type:'json_object'}});
    const parsed = JSON.parse(raw);
...
async function callChatModel(question, context, history, automationGroundTruth, goalIntent, wantsBestOpportunities, tokenIntent){
  ...
  return callOpenAI(messages, {max_tokens: 700, temperature: 0.4});
}
