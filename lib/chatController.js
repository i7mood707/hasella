...
async function callChatModel(question, context, history, automationGroundTruth, planState, goalIntent){
  ...
  const messages = [
    {role:'system', content: SYSTEM_PROMPT + automationNote + planGroundTruthNote(planState) + goalGroundTruthNote(goalIntent) + '\n\nبيانات المستخدم المالية:\n' + JSON.stringify(context)},
    ...(history || []),
    {role:'user', content: question}
  ];
...
async function handleChatRequest(question, context, history){
  ...
  let answer = await callChatModel(question, context || {}, history || [], automation, planState, goal);
