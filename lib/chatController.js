...
إذا طلب المستخدم خطة أو توصية استثمارية بالأسهم، أو أفضل الفرص الاستثمارية المتاحة: لا يوجد صندوق أو خطة جاهزة بنسب مخاطرة — بدلًا من ذلك حلّل الأسهم المتوفرة فعليًا واعتمد على منهجيات الاستثمار المعروفة لتقييمها وترتيبها، كما هو موضّح في أي تعليمات إضافية تُعطى لك عند الحاجة.
...
رتّب الأسهم المتوفرة من الأفضل للأسوأ فرصة حاليًا بناءً على مجموع هذه الفلسفات الست. لا تخصص فقرة منفصلة لكل مستثمر — وازن بين الفلسفات الست معًا بكل سهم، لكن اذكر أي مستثمر من الستة كان الأكثر تأثيرًا في ترتيب هذا السهم تحديدًا.
مهم جدًا بخصوص التنسيق: اكتب أول جملة قصيرة توضّح أن هذا تحليل عام مبني على منهجيات استثمارية معروفة وليس توصية استثمارية معتمدة، ثم سطر فارغ، ثم كل سهم في سطر مستقل تمامًا (افصل بين كل سهم والي بعده بسطر جديد حقيقي، ممنوع وضع الترتيب كله في فقرة واحدة متصلة). اكتب كل سطر بالصيغة التالية بالضبط:
رقم. **اسم السهم** (رمزه) — سبب موجز مبني على السعر والتغيّر المعطى. الأقرب لأسلوب: اسم المستثمر الأكثر تأثيرًا.`;
...
async function callChatModel(question, context, history, automationGroundTruth, goalIntent, wantsBestOpportunities){
  ...
  const messages = [
    {role:'system', content: SYSTEM_PROMPT + automationNote + goalGroundTruthNote(goalIntent) + bestOpportunityNote + '\n\nبيانات المستخدم المالية:\n' + JSON.stringify(context)},
    ...(history || []),
    {role:'user', content: question}
  ];
...
async function handleChatRequest(question, context, history){
  ...
  let answer = await callChatModel(question, context || {}, history || [], automation, goal, wantsBestOpportunities);
