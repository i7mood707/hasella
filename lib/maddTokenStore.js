const fs = require('fs');
const path = require('path');
const os = require('os');

const BLOB_PATHNAME = 'madd-tokens.json';
// __dirname sits inside the deployed bundle, which is read-only on Vercel — os.tmpdir()
// (/tmp) is the only writable path there, and works fine for local dev too.
const LOCAL_STORE_PATH = path.join(os.tmpdir(), 'hasella-madd-tokens.json');

function usingBlob(){
  return !!process.env.BLOB_READ_WRITE_TOKEN;
}

function readAllFromDisk(){
  try{
    return JSON.parse(fs.readFileSync(LOCAL_STORE_PATH, 'utf8'));
  }catch(err){
    return {};
  }
}

function writeAllToDisk(tokens){
  fs.mkdirSync(path.dirname(LOCAL_STORE_PATH), {recursive:true});
  fs.writeFileSync(LOCAL_STORE_PATH, JSON.stringify(tokens));
}

async function readAllFromBlob(){
  const { list } = require('@vercel/blob');
  const { blobs } = await list({prefix: BLOB_PATHNAME, limit: 1});
  const match = blobs.find(b => b.pathname === BLOB_PATHNAME);
  if(!match) return {};
  const res = await fetch(match.url, {cache:'no-store'});
  if(!res.ok) return {};
  return await res.json();
}

async function writeAllToBlob(tokens){
  const { put } = require('@vercel/blob');
  await put(BLOB_PATHNAME, JSON.stringify(tokens), {
    access: 'public',
    addRandomSuffix: false,
    allowOverwrite: true,
    contentType: 'application/json'
  });
}

function readAll(){
  return usingBlob() ? readAllFromBlob() : Promise.resolve(readAllFromDisk());
}

function writeAll(tokens){
  return usingBlob() ? writeAllToBlob(tokens) : Promise.resolve(writeAllToDisk(tokens));
}

async function createToken(code, {amount, accounts, isOneTime, expiresAt}){
  const tokens = await readAll();
  tokens[code] = {
    amount: Number(amount),
    accounts: accounts || [],
    isOneTime: !!isOneTime,
    expiresAt: expiresAt || null,
    used: false,
    usedAt: null,
    createdAt: Date.now()
  };
  await writeAll(tokens);
  return tokens[code];
}

async function getToken(code){
  const tokens = await readAll();
  return tokens[code] || null;
}

// Allocates the invoice amount across the token's backing accounts, smallest
// contribution first, so a partial redemption drains the small amounts before
// touching the larger ones. Whatever isn't consumed is the change owed back.
function allocateCharge(accounts, amount){
  const sorted = [...(accounts || [])].sort((a, b) => a.amt - b.amt);
  let remaining = Math.round(Number(amount) * 100);
  const charged = [];
  for(const acc of sorted){
    if(remaining <= 0) break;
    const accCents = Math.round(acc.amt * 100);
    const take = Math.min(accCents, remaining);
    if(take > 0) charged.push({bank: acc.bank, amt: take / 100});
    remaining -= take;
  }
  return charged;
}

async function redeemToken(code, amount, desc){
  const tokens = await readAll();
  const token = tokens[code];
  if(!token) return {ok:false, error:'not_found'};
  if(token.used) return {ok:false, error:'already_used'};
  if(token.expiresAt && Date.now() > token.expiresAt) return {ok:false, error:'expired'};
  const amountCents = Math.round(Number(amount)*100);
  const tokenCents = Math.round(Number(token.amount)*100);
  if(amountCents > tokenCents){
    return {ok:false, error:'amount_exceeds', expectedAmount: token.amount};
  }
  token.used = true;
  token.usedAt = Date.now();
  token.desc = desc || null;
  token.chargedAmount = amountCents / 100;
  token.changeAmount = (tokenCents - amountCents) / 100;
  token.chargedAccounts = allocateCharge(token.accounts, token.chargedAmount);
  tokens[code] = token;
  await writeAll(tokens);
  return {ok:true, token};
}

module.exports = { createToken, getToken, redeemToken };
